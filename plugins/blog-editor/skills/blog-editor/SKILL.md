...
사용자가 자연어 입력에 `blogId` 또는 블로그명을 명시하지 않은 경우, 다음 **순서대로** 파일을 찾아 첫 번째로 발견된 값을 **기본 대상 블로그**로 사용한다.
...
적용 규칙:
- 입력에 다른 블로그가 명시되면(이름·ID 매칭) 입력값을 우선한다.
- 첫 호출 직전 한 번 "기본 블로그 `<blogName>`을 사용합니다." 안내 후 진행 (재차 알릴 필요 없음).
- 두 파일 모두 없거나 `blogId`가 비어 있으면 **`GET /api/v1/blog`(내 블로그 목록, PAT 필수)** 로 후보를 좁힌 뒤, 모호하면 사용자 확인.
...
- **blogId 누락**: project.json → 없으면 `GET /api/v1/blog`로 후보 추출 → 모호하면 사용자 확인.
