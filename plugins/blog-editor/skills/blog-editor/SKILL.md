...
- 두 파일 모두 없거나 `blogId`가 비어 있으면 **`GET /api/v1/blog`(내 블로그 목록, PAT 필수)** 로 후보를 좁힌 뒤, 모호하면 사용자 확인. 확정 후 "이 블로그를 프로젝트 설정 파일(`./.claude/skills/blog-editor/project.json`)로 저장할까요?"를 제안할 수 있다.
