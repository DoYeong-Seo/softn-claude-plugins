...
- 컬럼 메타: `columnId`=물리명(`DOC_ID`), `columnName`=한글 논리명(`문서 아이디`) — `columnId` 에 UUID 금지.

## 명명 워크플로우 — 용어사전·도메인 통합

테이블·컬럼의 **생성·수정 의도가 감지되면 무조건** [NAMING.md](NAMING.md) 절차를 따른다. 모델이 임의로 물리명을 만들거나 도메인을 선택하지 않는다.

핵심 4원칙 (상세는 NAMING.md):
1. 물리명은 용어사전(Glossary) 매핑으로만 조립 — 임의 축약·번역 금지.
2. 사전에 없는 용어는 `AskUserQuestion` 으로 **사용자 동의 후에만** 등록.
3. 컬럼은 `domainId` 필수 — 후보 추천 후 **사용자가 직접 선택** (없음 선택지 포함).
4. **컬럼 메타 저장 규칙**: `columnId`=**물리명**(예: `DOC_ID`), `columnName`=**한글 논리명**(예: `문서 아이디`). **`columnId` 에 UUID 절대 금지** ([CONVENTIONS.md §4](CONVENTIONS.md)).

절차 요약: ① 논리명 분해 → ② 사전 조회·누락 등록 동의 → ③ 물리명(→`columnId`)·한글 논리명(→`columnName`) 조립 → ④ 도메인 후보 사용자 선택 → ⑤ 최종 요약 후 API 호출.
